...
-- Текст ожидания назначения клавиши: «Нажмите клавишу» + бегущие точки и пульс яркости.
-- Возвращает string, alpha_mul (0.55..1.0).
local function waiting_key_label(prefix)
    prefix = prefix or u8"Нажмите клавишу"
    local t = os.clock()
    local phase = math.floor(t * 3.0) % 4 -- 0..3
    local dots = ({ "", ".", "..", "..." })[phase + 1]
    local pulse = 0.55 + 0.45 * (math.sin(t * 3.6) * 0.5 + 0.5)
...
function menu.button(str_id, label, size, active, alpha_mult, dim, danger)
    ...
    local lsz = imgui.CalcTextSize(label)
    dl:AddText(imgui.ImVec2(p1.x + (p2.x - p1.x - lsz.x) / 2, p1.y + (p2.y - p1.y - lsz.y) / 2),
        imgui.ColorConvertFloat4ToU32(imgui.ImVec4(t.text.x, t.text.y, t.text.z, tx_a * a)), label)
...
function menu.draw_popup()
    ...
    if imgui.Begin("##mhg_popup", nil, flags) then
        ...
        elseif P.shown == "kb" then
            ...
            -- добавить клавишу по хоткею (текст ожидания — прямо в кнопке, как у хоткея меню)
            local add_active = KB.awaiting_new_key and true or false
            local add_lbl, add_pulse = u8"+ Добавить клавишу", 1.0
            if add_active then
                add_lbl, add_pulse = waiting_key_label(u8"Нажмите клавишу")
            end
            if menu.button("##pop_kb_add", add_lbl, full, add_active, an * add_pulse) and ready then
...
function menu.draw()
    ...
    if imgui.Begin('##menu', nil, imgui.WindowFlags.NoTitleBar + imgui.WindowFlags.NoResize + imgui.WindowFlags.NoScrollbar + imgui.WindowFlags.NoScrollWithMouse) then
        ...
        elseif menu.current_tab == 3 then
            ...
            local hk_str, hk_wait_pulse = get_key_name(session_cfg.settings.open_key), 1.0
            if menu.hotkey_listening then
                hk_str, hk_wait_pulse = waiting_key_label(u8"Нажмите клавишу")
            end
            ...
            local hk_text_val = lerp(lerp(0.65, 0.85, hotkey_btn_hover_alpha), 1.00, hk_fill_val) * hk_wait_pulse
...
keyboard_frame = imgui.OnFrame(
    ...
    function()
        ...
        -- подсказка в режиме правки (без подложки, только текст)
        if KB.edit_alpha > 0.01 then
            local hint, hint_pulse = u8"ЛКМ — выбрать/перетащить, колесо — размер, ✕ или Delete — удалить. ESC — выйти.", 1.0
            if KB.awaiting_new_key then
                local base, pulse = waiting_key_label(u8"Нажмите клавишу, чтобы добавить")
                hint = base .. u8"  ESC — отмена"
                hint_pulse = pulse
            end
            local hsz = imgui.CalcTextSize(hint)
            local hw, hh = hsz.x + 6, hsz.y + 6
            ...
            if imgui.Begin("##mhg_kb_hint", nil, base_flags + imgui.WindowFlags.NoInputs) then
                local dl = imgui.GetWindowDrawList()
                local wpos = imgui.GetWindowPos()
                local ha = KB.edit_alpha * a * hint_pulse
