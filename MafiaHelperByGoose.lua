...
local function get_theme()
    return themes[session_cfg.settings.theme or 1]
end
...
local function CustomSliderFloat(str_id, label, p_val, v_min, v_max, size)
    ...
    local bg_col = imgui.ColorConvertFloat4ToU32(imgui.ImVec4(bg_r, bg_g, bg_b, 0.40 * menu.alpha))
    ...
    local fill_col = imgui.ColorConvertFloat4ToU32(imgui.ImVec4(fill_r, fill_g, fill_b, 0.65 * menu.alpha))
    ...
    local bor_col = imgui.ColorConvertFloat4ToU32(imgui.ImVec4(bor_r, bor_g, bor_b, lerp(0.25, 0.95, hover_p) * menu.alpha))
    ...
    local txt_col = imgui.ColorConvertFloat4ToU32(imgui.ImVec4(t.text.x, t.text.y, t.text.z, lerp(0.65, 1.00, hover_p) * menu.alpha))
...
function menu.draw()
    ...
    local dynamic_win_width = math.max(780, math.max(content_elements_width_top, total_bottom_width) + base_margins)

    apply_window_position(dynamic_win_width, 420)
    ...
    if imgui.Begin('##menu', nil, imgui.WindowFlags.NoTitleBar + imgui.WindowFlags.NoResize + imgui.WindowFlags.NoScrollbar) then
        ...
        elseif menu.current_tab == 2 then
            ...
            if session_cfg.settings.asp_enabled then
                ...
                if imgui.InvisibleButton("##asp_reset_btn", reset_btn_size) then
                    asp_val_buf[0] = 1.25
...
local function check_update()
    local tmp = mhg_dir .. "\\version_check.txt"
    downloadUrlToFile(VERSION_URL .. "?t=" .. os.time(), tmp, function(id, status)
...
function update_ui.draw()
    ...
    if imgui.Begin("##update_ui", nil, imgui.WindowFlags.NoTitleBar + imgui.WindowFlags.NoResize + imgui.WindowFlags.NoScrollbar) then
        ...
        if update_ui.stage == "loading" then
            ...
            local load_dur = 5.5
            local target_p = math.min(1.0, elapsed / load_dur)
            update_ui.load_progress = lerp(update_ui.load_progress or 0, target_p, 0.08)
            if update_ui.pending and elapsed >= load_dur then
        ...
        elseif update_ui.stage == "loading" then
            ...
            local spinner_center = imgui.ImVec2(pos.x + 170, pos.y + 70)
            local spinner_color = imgui.ColorConvertFloat4ToU32(imgui.ImVec4(t.accent[1], t.accent[2], t.accent[3], sa))
            draw_spinner(draw_list, spinner_center, 16.0, 2.8, spinner_color)

            imgui.Dummy(imgui.ImVec2(0, 52))

            local bar_w, bar_h = 220.0, 8.0
            local bx = pos.x + (340 - bar_w) * 0.5
            local by = imgui.GetCursorScreenPos().y
            local bg_col = imgui.ColorConvertFloat4ToU32(imgui.ImVec4(t.bg_idle[1], t.bg_idle[2], t.bg_idle[3], 0.60 * sa))
            local fill_col = imgui.ColorConvertFloat4ToU32(imgui.ImVec4(t.accent[1], t.accent[2], t.accent[3], 0.90 * sa))
            local bor_col = imgui.ColorConvertFloat4ToU32(imgui.ImVec4(t.bor_hover[1], t.bor_hover[2], t.bor_hover[3], 0.55 * sa))
            draw_list:AddRectFilled(imgui.ImVec2(bx, by), imgui.ImVec2(bx + bar_w, by + bar_h), bg_col, 4.0)
            local fw = bar_w * math.max(0.03, math.min(1.0, update_ui.load_progress or 0))
            if fw > 0 then
            ...
            imgui.SetCursorPosX((340 - txt_sz.x) * 0.5)
