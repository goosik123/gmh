...
local session_cfg = inicfg.load({
    session = {
        ...
        saved_nick = "",
        saved_password = "",
        saved_uid = ""  -- постоянный UID аккаунта (цифры); надёжнее ника при перелётах
    },
    settings = {
        ...
        watermark_show_title = true,  -- "MHG"
        watermark_show_nick = true,
        watermark_show_fps = true,
        watermark_show_ping = true,
        watermark_show_time = true,
...
-- На ARZ при перелёте на VC (и в /id) вместо ника кратко отдаётся UID аккаунта — строка из цифр.
-- Session ID в скобках [0..1000] сюда не попадает: клиент подставляет именно account UID.
local function is_account_uid(str)
    if not str or str == "" or str == "Unknown" then return false end
    local s = tostring(str):match("^%s*(.-)%s*$") or ""
    -- UID аккаунта обычно длиннее слот-id; берём от 4 цифр, чтобы не путать с 0..999
    return s:match("^%d+$") ~= nil and #s >= 4
end

local function is_temp_uid_nick(nick)
    return is_account_uid(nick)
end

-- Обновить saved_uid, если сейчас вместо ника виден UID
local function maybe_capture_uid_from_nick(nick)
    if not is_account_uid(nick) then return end
    local uid = tostring(nick):match("^%s*(.-)%s*$")
    if uid and uid ~= "" and (session_cfg.session.saved_uid or "") ~= uid then
        session_cfg.session.saved_uid = uid
...
-- стиль свечения до включения радужной темы (чтобы вернуть при выходе)
local glow_style_before_rainbow = nil
...
-- key в accounts.txt может быть ником ИЛИ UID (цифры) — проверяем оба варианта
local function check_account(key, password)
    if not key or key == "" or not password or password == "" then return false end
    if auth.account_map and auth.account_map[key] then
        return auth.account_map[key] == password
    end
    for _, acc in ipairs(auth.accounts) do
        if acc.nick == key and acc.password == password then return true end
    end
    return false
end

-- есть ли доступ по сохранённой сессии (ник и/или UID)
local function session_credentials_valid(saved_nick, saved_uid, saved_pass)
    if not saved_pass or saved_pass == "" then return false end
    if saved_uid ~= "" and check_account(saved_uid, saved_pass) then return true end
    if saved_nick ~= "" and check_account(saved_nick, saved_pass) then return true end
    return false
end
...
-- идентификация: приоритет UID, ник — запасной. Временный UID-ник не сбрасывает сессию.
local function validate_logged_session()
    ...
    local saved_uid = session_cfg.session.saved_uid or ""
    local saved_pass = session_cfg.session.saved_password or ""

    -- пароль/аккаунт убрали из списка
    if not session_credentials_valid(saved_nick, saved_uid, saved_pass) then
        force_logout()
        return false
    end

    -- сейчас вместо ника — UID: запоминаем и не разлогиниваем
    if is_temp_uid_nick(nick) then
        maybe_capture_uid_from_nick(nick)
        local uid = tostring(nick):match("^%s*(.-)%s*$")
        if saved_uid ~= "" and uid ~= saved_uid then
            if not check_account(uid, saved_pass) then
                force_logout()
                return false
            end
            session_cfg.session.saved_uid = uid
    ...
    -- обычный ник: несовпадение с сохранённым (и нет валидного UID-бэка) → выход
    if saved_nick ~= "" and nick ~= saved_nick and nick ~= "Unknown" then
        if saved_uid ~= "" and check_account(saved_uid, saved_pass) then
    ...
    if saved_nick == "" and saved_uid == "" then
        force_logout()
        return false
    end
        ...
        if auth.stage == 'idle' then
            ...
            if imgui.InvisibleButton('##custom_login_btn', base_size) then
                if auth.loaded and has_text then
                    ...
                    local nick = get_my_nick()
                    local ok = check_account(nick, pass)
                    local login_uid = ""

                    if is_account_uid(nick) then
                        login_uid = tostring(nick):match("^%s*(.-)%s*$") or ""
                        ok = ok or check_account(login_uid, pass)
                    end

                    if ok then
                        session_cfg.session.is_logged = true
                        if is_account_uid(nick) then
                            session_cfg.session.saved_uid = login_uid
                            if (session_cfg.session.saved_nick or "") == "" then
                                session_cfg.session.saved_nick = login_uid
                            end
                        else
                            session_cfg.session.saved_nick = nick
                        end
...
function menu.draw_popup()
    ...
    if imgui.Begin("##mhg_popup", nil, flags) then
        ...
        if P.shown == "wm" then
            ...
            imgui.Dummy(imgui.ImVec2(0, 8))
            menu.anim_text("##pop_wm_parts_lbl", u8"Отображение:", t.text_muted, ct)
            imgui.Dummy(imgui.ImVec2(0, 4))

            -- переключатели частей watermark (MHG всегда включён)
            -- Ник/FPS в ряд; Пинг и Время — на всю ширину
            local part_toggles = {
                { key = "watermark_show_nick",  label = u8"Ник" },
                { key = "watermark_show_fps",   label = "FPS" },
            }
            local pt_gap = 8
            local pt_cell = (CONTENT_W - pt_gap) / 2
            local pt_sz = imgui.ImVec2(pt_cell, 32)
            for i, pt in ipairs(part_toggles) do
                local enabled = cfg[pt.key] ~= false
                if menu.button("##pop_wm_part_" .. pt.key, pt.label, pt_sz, enabled, an) and ready then
                    cfg[pt.key] = not enabled
                    inicfg.save(session_cfg, CONFIG_FILE)
                end
                if i % 2 == 1 then
                    imgui.SameLine(0, pt_gap)
                else
                    imgui.Dummy(imgui.ImVec2(0, 4))
                end
            end
            local ping_on = cfg.watermark_show_ping ~= false
            if menu.button("##pop_wm_part_watermark_show_ping", u8"Пинг", full, ping_on, an) and ready then
                cfg.watermark_show_ping = not ping_on
                inicfg.save(session_cfg, CONFIG_FILE)
            end
            imgui.Dummy(imgui.ImVec2(0, 4))
            local time_on = cfg.watermark_show_time ~= false
            if menu.button("##pop_wm_part_watermark_show_time", u8"Время", full, time_on, an) and ready then
                cfg.watermark_show_time = not time_on
                inicfg.save(session_cfg, CONFIG_FILE)
            end
        ...
        elseif P.shown == "glow" then
            ...
            local half = imgui.ImVec2((CONTENT_W - 10) / 2, 32)
            local theme_is_rainbow = themes[cfg.theme or 1] and themes[cfg.theme or 1].rainbow
            -- при радужной теме режим «Радуга» скрыт (он уже по умолчанию)
            local glow_choices = {}
            for i, name in ipairs(GLOW_STYLES) do
                if not (theme_is_rainbow and i == 3) then
                    glow_choices[#glow_choices + 1] = { idx = i, name = name }
                end
            end
            local cur_style = math.max(1, math.min(math.floor(cfg.window_glow_style or 1), #GLOW_STYLES))
            for i, ch in ipairs(glow_choices) do
                -- при радужной теме (3 пункта) последний — на всю ширину как 2 кнопки
                local btn_sz = (theme_is_rainbow and i == #glow_choices) and full or half
                if menu.button("##pop_glow_style_" .. ch.idx, ch.name, btn_sz, cur_style == ch.idx, an) and ready then
                    cfg.window_glow_style = ch.idx
                    inicfg.save(session_cfg, CONFIG_FILE)
                end
                if theme_is_rainbow then
                    if i < #glow_choices and i % 2 == 1 then
                        imgui.SameLine(0, 10)
                    else
                        imgui.Dummy(imgui.ImVec2(0, 4))
                    end
                else
                    if i % 2 == 1 and i < #glow_choices then
                        imgui.SameLine(0, 10)
                    else
                        imgui.Dummy(imgui.ImVec2(0, 4))
                    end
...
function menu.draw()
    ...
    if imgui.Begin('##menu', nil, imgui.WindowFlags.NoTitleBar + imgui.WindowFlags.NoResize + imgui.WindowFlags.NoScrollbar + imgui.WindowFlags.NoScrollWithMouse) then
        ...
        elseif menu.current_tab == 2 then
            menu.anim_text("##tab2_asp_lbl", u8"Соотношение экрана:", t.text_muted, 1.0)
            imgui.Dummy(imgui.ImVec2(0, 5))
        ...
        elseif menu.current_tab == 3 then
            ...
            for i, theme_data in ipairs(themes) do
                ...
                if imgui.InvisibleButton("##custom_theme_btn_" .. i, theme_btn_size) then
                    local prev_theme = session_cfg.settings.theme or 1
                    local was_rainbow = themes[prev_theme] and themes[prev_theme].rainbow
                    local now_rainbow = themes[i] and themes[i].rainbow

                    session_cfg.settings.theme = i

                    if now_rainbow and not was_rainbow then
                        -- вход в радужную тему: запоминаем текущий стиль и ставим радугу
                        local cur = session_cfg.settings.window_glow_style or 1
                        if cur ~= 3 then
                            glow_style_before_rainbow = cur
                        end
                        session_cfg.settings.window_glow_style = 3
                    elseif was_rainbow and not now_rainbow then
                        -- выход из радужной темы: возвращаем прошлый режим
                        local restore = glow_style_before_rainbow or 1
                        if restore == 3 then restore = 1 end
                        session_cfg.settings.window_glow_style = restore
                        glow_style_before_rainbow = nil
                    end
...
watermark_frame = imgui.OnFrame(
    ...
    function()
        ...
        local wcfg = session_cfg.settings
        local parts = { "MHG" } -- всегда
        if wcfg.watermark_show_nick ~= false then
            table.insert(parts, nick)
        end
        if wcfg.watermark_show_fps ~= false then
            table.insert(parts, string.format("%d fps", fps))
        end
        if wcfg.watermark_show_ping ~= false then
            table.insert(parts, string.format("%d ms", ping))
        end
        if wcfg.watermark_show_time ~= false then
        ...
        -- smooth resize when content width changes
        ...
        if imgui.Begin("##watermark_overlay", nil, wm_flags) then
            ...
            if session_cfg.settings.window_glow_enabled then
                ...
                local gr, gg, gb = t.accent[1], t.accent[2], t.accent[3]
                if gstyle == 3 then
            ...
            for i, part in ipairs(parts) do
                ...
                -- MHG — акцент, остальное — обычный цвет текста
                local col = (i == 1) and accent_col or text_col
