...
function menu.draw()
    ...
    if imgui.Begin('##menu', nil, imgui.WindowFlags.NoTitleBar + imgui.WindowFlags.NoResize + imgui.WindowFlags.NoScrollbar) then
        ...
        if menu.current_tab == 1 then
            ...
            if rv > 0.01 then
                ...
                if KB.edit then
                    ...
                    -- === Добавить клавишу (захват хоткея) ===
                    local p_add = imgui.GetCursorScreenPos()
                    if imgui.InvisibleButton("##mhg_kb_add_btn", kb_btn_size) then
                        KB.awaiting_new_key = not KB.awaiting_new_key
                    end

                    KB.add_hover = lerp(KB.add_hover, imgui.IsItemHovered() and 1.0 or 0.0, 0.14)
                    KB.add_fill = lerp(KB.add_fill, KB.awaiting_new_key and 1.0 or 0.0, 0.12)

                    local add_bg_r = lerp(lerp(t.bg_idle[1], t.bg_hover[1], KB.add_hover), t.bg_act[1], KB.add_fill)
                    local add_bg_g = lerp(lerp(t.bg_idle[2], t.bg_hover[2], KB.add_hover), t.bg_act[2], KB.add_fill)
                    local add_bg_b = lerp(lerp(t.bg_idle[3], t.bg_hover[3], KB.add_hover), t.bg_act[3], KB.add_fill)
                    local add_bg_a = lerp(lerp(0.20, 0.45, KB.add_hover), 0.75, KB.add_fill)
                    local add_br_r = lerp(lerp(t.bor_idle[1], t.bor_hover[1], KB.add_hover), t.bor_act[1], KB.add_fill)
                    local add_br_g = lerp(lerp(t.bor_idle[2], t.bor_hover[2], KB.add_hover), t.bor_act[2], KB.add_fill)
                    local add_br_b = lerp(lerp(t.bor_idle[3], t.bor_hover[3], KB.add_hover), t.bor_act[3], KB.add_fill)
                    local add_br_a = lerp(lerp(0.25, 0.60, KB.add_hover), 0.95, KB.add_fill)

                    draw_list:AddRectFilled(p_add, imgui.ImVec2(p_add.x + kb_btn_size.x, p_add.y + kb_btn_size.y),
                        imgui.ColorConvertFloat4ToU32(imgui.ImVec4(add_bg_r, add_bg_g, add_bg_b, add_bg_a * rv_alpha)), 6.0)
                    draw_list:AddRect(p_add, imgui.ImVec2(p_add.x + kb_btn_size.x, p_add.y + kb_btn_size.y),
                        imgui.ColorConvertFloat4ToU32(imgui.ImVec4(add_br_r, add_br_g, add_br_b, add_br_a * rv_alpha)), 6.0, 15, 1.2)

                    local add_lbl = KB.awaiting_new_key and u8"Нажмите клавишу..." or u8"+ Добавить клавишу"
                    local add_lbl_sz = imgui.CalcTextSize(add_lbl)
                    local add_txt_val = lerp(lerp(0.65, 0.85, KB.add_hover), 1.00, KB.add_fill)
                    draw_list:AddText(imgui.ImVec2(p_add.x + (kb_btn_size.x - add_lbl_sz.x) / 2, p_add.y + (kb_btn_size.y - add_lbl_sz.y) / 2),
                        imgui.ColorConvertFloat4ToU32(imgui.ImVec4(t.text.x, t.text.y, t.text.z, add_txt_val * rv_alpha)), add_lbl)

                    -- === Панель выбранной клавиши: размер и удаление ===
                    if KB.selected_id then
                        ...
                        if sel_key then
                            ...
                            imgui.TextColored(imgui.ImVec4(t.text_muted.x, t.text_muted.y, t.text_muted.z, t.text_muted.w * rv),
                                u8"Выбрана: " .. (sel_key.label or sel_key.id))
                            imgui.Dummy(imgui.ImVec2(0, 5))

                            KB.size_buf[0] = sel_key.s
                            CustomSliderFloat("##mhg_kb_size_slider", u8"Размер", KB.size_buf, KB.SCALE_MIN, KB.SCALE_MAX, kb_btn_size, rv_alpha)
                            ...
                            imgui.SameLine(0, kb_spacing_x)
                            local p_del = imgui.GetCursorScreenPos()
                            if imgui.InvisibleButton("##mhg_kb_del_btn", kb_btn_size) then
                                KB.remove_key(sel_key.id)
                            end

                            KB.del_hover = lerp(KB.del_hover, imgui.IsItemHovered() and 1.0 or 0.0, 0.14)

                            local del_bg_r = lerp(t.bg_idle[1], 0.45, KB.del_hover)
                            local del_bg_g = lerp(t.bg_idle[2], 0.12, KB.del_hover)
                            local del_bg_b = lerp(t.bg_idle[3], 0.12, KB.del_hover)
                            local del_bg_a = lerp(0.20, 0.75, KB.del_hover)
                            local del_br_r = lerp(t.bor_idle[1], 0.85, KB.del_hover)
                            local del_br_g = lerp(t.bor_idle[2], 0.25, KB.del_hover)
                            local del_br_b = lerp(t.bor_idle[3], 0.25, KB.del_hover)

                            draw_list:AddRectFilled(p_del, imgui.ImVec2(p_del.x + kb_btn_size.x, p_del.y + kb_btn_size.y),
                                imgui.ColorConvertFloat4ToU32(imgui.ImVec4(del_bg_r, del_bg_g, del_bg_b, del_bg_a * rv_alpha)), 6.0)
                            draw_list:AddRect(p_del, imgui.ImVec2(p_del.x + kb_btn_size.x, p_del.y + kb_btn_size.y),
                                imgui.ColorConvertFloat4ToU32(imgui.ImVec4(del_br_r, del_br_g, del_br_b, lerp(0.25, 0.95, KB.del_hover) * rv_alpha)), 6.0, 15, 1.2)

                            local del_lbl = u8"Удалить"
                            local del_lbl_sz = imgui.CalcTextSize(del_lbl)
                            draw_list:AddText(imgui.ImVec2(p_del.x + (kb_btn_size.x - del_lbl_sz.x) / 2, p_del.y + (kb_btn_size.y - del_lbl_sz.y) / 2),
                                imgui.ColorConvertFloat4ToU32(imgui.ImVec4(t.text.x, t.text.y, t.text.z, lerp(0.75, 1.00, KB.del_hover) * rv_alpha)), del_lbl)
                        else
                            KB.selected_id = nil
                        end
                    end
        ...
        elseif menu.current_tab == 2 then
            imgui.TextColored(t.text_muted, u8"Соотношение сторон:")
            imgui.Dummy(imgui.ImVec2(0, 5))
...
KB = {
    ...
    awaiting_new_key = false, -- ждём нажатия клавиши, чтобы добавить её на оверлей
...
local function kb_add_key(vk)
    ...
    local new_key = {
        id = id, kind = "c", vk = vk, label = get_key_name(vk), glyph = nil, bw = nil,
...
keyboard_frame = imgui.OnFrame(
    ...
    function()
        ...
        for _, k in ipairs(KB.keys) do
            -- ширина добавленных пользователем клавиш считается один раз по тексту метки
            if not k.bw then
                imgui.SetWindowFontScale(1.0)
                local lbl_sz = imgui.CalcTextSize(k.label or "?")
                k.bw = math.max(KB_UNIT, lbl_sz.x + 16)
            end
        ...
        if KB.edit_alpha > 0.01 then
            local hint = KB.awaiting_new_key
                and u8"Нажмите клавишу, чтобы добавить её. ESC — отмена."
